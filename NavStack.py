...
class Map:
    ...
    def __init__(self, map, map_meters=35):
        ...
        if isinstance(map, str):
            ...
            elif map.split('.')[-1] in ["jpeg", "png", "jpg"]:
    ...
    def isValidPoint(self, point):
        bounds_check = (0 <= point[1] < self.map.shape[0]) and (0 <= point[0] < self.map.shape[1])
        return bounds_check and not self.map[point[1], point[0]]
